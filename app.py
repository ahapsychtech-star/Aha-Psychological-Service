...
def notify_daily_schedule(date_str=None):
    ...
    with get_db() as conn:
        therapists = conn.execute("""
            ...
            WHERE u.role='therapist' AND u.is_active=1 AND date(a.start_time)=date(?)
        """, (date_str,)).fetchall()
        for therapist in therapists:
            appts = conn.execute("""
                ...
                WHERE a.therapist_id=? AND date(a.start_time)=date(?) AND a.status='scheduled'
...
def get_dashboard_analytics():
    ...
    with get_db() as conn:
        ...
        new_this_month = conn.execute("SELECT COUNT(*) FROM clients WHERE registration_date >= date('now','start of month')").fetchone()[0]
        today_appts = conn.execute("SELECT COUNT(*) FROM appointments WHERE date(start_time)=date('now') AND status='scheduled'").fetchone()[0]
        ...
        revenue_this_month = conn.execute("SELECT COALESCE(SUM(amount_paid),0) FROM payments WHERE payment_date >= date('now','start of month')").fetchone()[0]
